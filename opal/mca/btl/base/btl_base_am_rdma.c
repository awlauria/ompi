...
static OBJ_CLASS_INSTANCE(mca_btl_base_am_rdma_module_t, opal_object_t,
                          mca_btl_base_am_rdma_module_init,
                          mca_btl_base_am_rdma_module_fini);
...
static inline
size_t mca_btl_base_rdma_operation_size (mca_btl_base_module_t *btl, mca_btl_base_rdma_type_t type,
                                         size_t remaining)
{
    switch (type) {
...
static void mca_btl_base_copy_from_segments (uint64_t addr, size_t skip_bytes,
    ...
    if (seg0_len > 0) {
        BTL_VERBOSE(("unpacking %" PRIsize_t " bytes from segment 0 to %p", seg0_len, (void *) addr));
...
static void mca_btl_base_copy_to_segments (uint64_t addr, size_t max_len, size_t skip_bytes,
    ...
    if (seg0_len > 0) {
        BTL_VERBOSE(("packing %" PRIsize_t " bytes from 0x%" PRIx64 " to segment 0",
                     seg0_len, addr));
...
static inline
int mca_btl_base_am_rdma_advance (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                  mca_btl_base_rdma_context_t *context, bool send_descriptor)
{
    ...
    mca_btl_base_rdma_hdr_t *hdr = (mca_btl_base_rdma_hdr_t *) descriptor->des_segments[0].seg_addr.pval;
                                                ...
                                                mca_btl_base_descriptor_t *descriptor, int status)
{
    ...
    (void) mca_btl_base_am_rdma_advance (btl, endpoint, (mca_btl_base_rdma_context_t *) descriptor->des_context, /*send_descriptor=*/false);
}


static inline int
mca_btl_base_rdma_start (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint, int type,
			 uint64_t operand1, uint64_t operand2, mca_btl_base_atomic_op_t op, int order,
			 int flags, size_t size, void *local_address, mca_btl_base_registration_handle_t *local_handle,
			 ...
			 mca_btl_base_rdma_completion_fn_t cbfunc, void *cbcontext, void *cbdata)
{
    ...
    BTL_VERBOSE(("Initiating RDMA operation. context=%p, size=%" PRIsize_t ", packet_size=%" PRIsize_t,
                 context, size, packet_size));

    descriptor = btl->btl_alloc (btl, endpoint, order, packet_size, MCA_BTL_DES_SEND_ALWAYS_CALLBACK |
...
static mca_btl_base_rdma_operation_t *mca_btl_base_rdma_alloc_operation (mca_btl_base_module_t *btl,
                                                                         struct mca_btl_base_endpoint_t *endpoint,
                                                                         mca_btl_base_descriptor_t *descriptor,
                                                                         const mca_btl_base_rdma_hdr_t *hdr)
{
    ...
    if (!mca_btl_base_rdma_is_atomic (hdr->type) && hdr->data.rdma.use_rdma && btl->btl_register_mem) {
...
static void mca_btl_base_rdma_queue_operation (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                               mca_btl_base_descriptor_t *descriptor, uint64_t atomic_response,
                                               const mca_btl_base_rdma_hdr_t *hdr, mca_btl_base_rdma_operation_t *operation) {
    if (NULL == operation) {
...
static int mca_btl_base_am_rdma_respond (mca_btl_base_module_t *btl,
    ...
    *descriptor = NULL;
    ...
    if (NULL == send_descriptor) {
        ...
        size_t data_size = mca_btl_base_rdma_is_atomic (hdr->type) ? hdr->data.atomic.size : hdr->data.rdma.size;
        size_t packet_size = sizeof (*resp_hdr) + (addr ? data_size : 0);
        send_descriptor = btl->btl_alloc (btl, endpoint, MCA_BTL_NO_ORDER, packet_size, MCA_BTL_DES_FLAGS_BTL_OWNERSHIP);
        if (NULL == send_descriptor) {
            return OPAL_ERR_OUT_OF_RESOURCE;
        }

        resp_hdr = (mca_btl_base_rdma_response_hdr_t *) send_descriptor->des_segments[0].seg_addr.pval;
        ...
        if (NULL != addr) {
            mca_btl_base_copy_to_segments ((uint64_t)(uintptr_t) addr, packet_size, sizeof (*resp_hdr),
                                           send_descriptor->des_segments, send_descriptor->des_segment_count);
        }
    }
...
static void mca_btl_base_am_rmda_rdma_complete (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                                void *local_address, struct mca_btl_base_registration_handle_t *local_handle,
    ...
    int ret = mca_btl_base_am_rdma_respond (operation->btl, operation->endpoint, &operation->descriptor,
                                            NULL, &operation->hdr);
    if (OPAL_UNLIKELY(OPAL_SUCCESS != ret)) {
        BTL_VERBOSE(("could not send a response. queueing the response for later. endpoint=%p, ret=%d", endpoint, ret));
...
static int mca_btl_base_am_rdma_target_get (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                            mca_btl_base_descriptor_t **descriptor, void *target_address,
                                            const mca_btl_base_rdma_hdr_t *hdr,
    ...
    if (hdr->data.rdma.use_rdma) {
        ...
        int ret = btl->btl_put (btl, endpoint, target_address, hdr->data.rdma.initiator_address,
...
static int mca_btl_base_am_rdma_target_put (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                            mca_btl_base_descriptor_t **descriptor, const mca_btl_base_segment_t *segments,
                                            size_t segment_count, void *target_address, const mca_btl_base_rdma_hdr_t *hdr,
                                            mca_btl_base_rdma_operation_t **operation) {
    if (hdr->data.rdma.use_rdma) {
        if (NULL == *operation) {
            *operation = mca_btl_base_rdma_alloc_operation (btl, endpoint, *descriptor, hdr);
        ...
        /* btl supports put but not get. emulating get with put */
        OBJ_RETAIN(*operation);
        int ret = btl->btl_get (btl, endpoint, target_address, hdr->data.rdma.initiator_address,
                                (struct mca_btl_base_registration_handle_t *) (*operation)->local_handle_data,
                                (struct mca_btl_base_registration_handle_t *) (*operation)->remote_handle_data,
                                hdr->data.rdma.size, /*flags=*/0, MCA_BTL_NO_ORDER, mca_btl_base_am_rmda_rdma_complete,
                                operation, NULL); 
    ...
    if (!operation->descriptor && !operation->is_completed) {
        ...
        case MCA_BTL_BASE_AM_GET:
            ret = mca_btl_base_am_rdma_target_get (operation->btl, operation->endpoint,
                                                   &operation->descriptor, target_address, &operation->hdr, &operation);
            break;
        case MCA_BTL_BASE_AM_PUT:
            ret = mca_btl_base_am_rdma_target_put (operation->btl, operation->endpoint,
                                                   &operation->descriptor, /*segments=*/NULL,
                                                   /*segment_count=*/0, target_address, &operation->hdr, &operation);
            break;
        case MCA_BTL_BASE_AM_ATOMIC:
            /* atomic operation was completed */
            ret = mca_btl_base_am_rdma_respond (operation->btl, operation->endpoint, &operation->descriptor,
                                                &operation->atomic_response, &operation->hdr);
            break;
        }
    } else {
        ret = mca_btl_base_am_rdma_respond (operation->btl, operation->endpoint, &operation->descriptor,
...
static int mca_btl_base_am_rdma_progress (void) {
    ...
    OPAL_THREAD_SCOPED_LOCK(&default_module.mutex,
                            ({
                                ...
                                OPAL_LIST_FOREACH_SAFE(operation, next, &default_module.queued_responses, mca_btl_base_rdma_operation_t) {
    ...
    OPAL_THREAD_SCOPED_LOCK(&default_module.mutex,
                            ({
                                ...
                                OPAL_LIST_FOREACH_SAFE(descriptor, next, &default_module.queued_initiator_descriptors, mca_btl_base_am_rdma_queued_descriptor_t) {
                                    mca_btl_base_rdma_context_t *context = (mca_btl_base_rdma_context_t *) descriptor->descriptor->des_context;
                                    int ret = descriptor->btl->btl_send (descriptor->btl,
                                                                         descriptor->endpoint,
                                                                         descriptor->descriptor,
                                                                         mca_btl_base_rdma_tag (context->type));
                                    if (OPAL_SUCCESS == ret) {
                                        opal_list_remove_item (&default_module.queued_initiator_descriptors, &descriptor->super);
                                    }
                                }
                            }));
}
...
static void mca_btl_base_am_rdma_response (mca_btl_base_module_t *btl, const mca_btl_base_receive_descriptor_t *desc) {
    mca_btl_base_rdma_response_hdr_t *resp_hdr = (mca_btl_base_rdma_response_hdr_t *) desc->des_segments[0].seg_addr.pval;

    assert (desc->des_segments[0].seg_len >= sizeof (*resp_hdr));

    mca_btl_base_rdma_context_t *context = (mca_btl_base_rdma_context_t *)(uintptr_t) resp_hdr->context;

    BTL_VERBOSE(("received response for RDMA operation. context=%p, size=%" PRIu64, context, resp_hdr->response_size));

    if (MCA_BTL_BASE_AM_PUT != context->type) {
        uint64_t local_address = resp_hdr->initiator_address ? resp_hdr->initiator_address : (uintptr_t) context->local_address;
        if (local_address && MCA_BTL_BASE_AM_PUT != context->type) {
            BTL_VERBOSE(("unpacking response from packet"));
            /* if there is a result copy it out of the incoming buffer. if RDMA is being used (get/put or put/get)
             * then the header should be the only thing in the incoming message. */
            mca_btl_base_copy_from_segments (local_address, sizeof (*resp_hdr), desc->des_segments,
                                             desc->des_segment_count);
        }
    }

    if (context->total_size == opal_atomic_add_fetch_64 (&context->acknowledged, resp_hdr->response_size)) {
...
static void mca_btl_base_am_process_rdma (mca_btl_base_module_t *btl, const mca_btl_base_receive_descriptor_t *desc)
{
    ...
    const mca_btl_base_rdma_hdr_t *hdr = (mca_btl_base_rdma_hdr_t *) desc->des_segments[0].seg_addr.pval;
    ...
    BTL_VERBOSE(("got active-message \"RDMA\" request. hdr->context=0x%" PRIx64 ", target_address=%p, "
    ...
    if (MCA_BTL_BASE_AM_PUT == hdr->type) {
        ret = mca_btl_base_am_rdma_target_put (btl, desc->endpoint, &descriptor, desc->des_segments,
                                               desc->des_segment_count, target_address, hdr, &operation);
...
static void mca_btl_base_am_process_atomic (mca_btl_base_module_t *btl, const mca_btl_base_receive_descriptor_t *desc)
{
    /* not all btls work with these active message atomics. at this time
     * all of the affected btls already have atomic support so there is
     * no need to attempt to get the endpoint here. */
    ...
    case MCA_BTL_BASE_AM_CAS:
        if (4 == hdr->data.atomic.size) {
            ...
            opal_atomic_compare_exchange_strong_32 ((opal_atomic_int32_t *) hdr->target_address, &tmp,
                                                    (int32_t) hdr->data.atomic.operand[1]);
            atomic_response = tmp;
        } if (8 == hdr->data.atomic.size) {
            opal_atomic_compare_exchange_strong_64 ((opal_atomic_int64_t *) hdr->target_address, &atomic_response,
                                                    hdr->data.atomic.operand[1]);
        }
        break;
    default:
    ...
    if (OPAL_SUCCESS != ret) {
        mca_btl_base_rdma_queue_operation (btl, desc->endpoint, descriptor, atomic_response,
                                           hdr, NULL);
    }
}
...
{
    mca_btl_base_active_message_trigger[MCA_BTL_BASE_TAG_RDMA].cbfunc = mca_btl_base_am_process_rdma;
    mca_btl_base_active_message_trigger[MCA_BTL_BASE_TAG_ATOMIC].cbfunc = mca_btl_base_am_process_atomic;
    mca_btl_base_active_message_trigger[MCA_BTL_BASE_TAG_RDMA_RESP].cbfunc = mca_btl_base_am_rdma_response;
}

static int mca_btl_base_am_fop (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                 ...
                                 void *cbcontext, void *cbdata)
{
    ...
    return mca_btl_base_rdma_start (btl, endpoint, MCA_BTL_BASE_AM_ATOMIC, operand, 0, op, order, flags,
                                    size, local_address, local_handle, remote_address, remote_handle,
                                    cbfunc, cbcontext, cbdata);
}

static int mca_btl_base_am_cswap (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                  void *local_address, uint64_t remote_address,
                                  mca_btl_base_registration_handle_t *local_handle,
                                  ...
                                  void *cbcontext, void *cbdata)
{
    ...
    return mca_btl_base_rdma_start (btl, endpoint, MCA_BTL_BASE_AM_CAS, compare, value, 0, order,
                                    flags, size, local_address, local_handle, remote_address, remote_handle,
                                    cbfunc, cbcontext, cbdata);
}

static int mca_btl_base_am_rdma_get (struct mca_btl_base_module_t *btl,struct mca_btl_base_endpoint_t *endpoint,
                                     void *local_address, uint64_t remote_address,
                                     struct mca_btl_base_registration_handle_t *local_handle,
                                     struct mca_btl_base_registration_handle_t *remote_handle, size_t size, int flags,
                                     int order, mca_btl_base_rdma_completion_fn_t cbfunc, void *cbcontext, void *cbdata)
{
    return mca_btl_base_rdma_start (btl, endpoint, MCA_BTL_BASE_AM_GET, 0, 0, 0, order, flags, size, local_address,
                                    local_handle, remote_address, remote_handle, cbfunc, cbcontext, cbdata);
}

static int mca_btl_base_am_rdma_put (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                     void *local_address, uint64_t remote_address,
                                     struct mca_btl_base_registration_handle_t *local_handle,
                                     struct mca_btl_base_registration_handle_t *remote_handle, size_t size, int flags,
                                     int order, mca_btl_base_rdma_completion_fn_t cbfunc, void *cbcontext, void *cbdata)
{
    return mca_btl_base_rdma_start (btl, endpoint, MCA_BTL_BASE_AM_PUT, 0, 0, 0, order, flags, size, local_address,
                                    local_handle, remote_address, remote_handle, cbfunc, cbcontext, cbdata);
}
...
int mca_btl_base_am_rdma_init (mca_btl_base_module_t *btl) {
    ...
    if (!(btl->btl_flags & MCA_BTL_FLAGS_ATOMIC_FOPS)) {
        ...
        btl->btl_atomic_flags = MCA_BTL_ATOMIC_SUPPORTS_GLOB |
            MCA_BTL_ATOMIC_SUPPORTS_CSWAP | MCA_BTL_ATOMIC_SUPPORTS_32BIT |
            MCA_BTL_ATOMIC_SUPPORTS_ADD | MCA_BTL_ATOMIC_SUPPORTS_AND |
            MCA_BTL_ATOMIC_SUPPORTS_OR | MCA_BTL_ATOMIC_SUPPORTS_XOR |
            MCA_BTL_ATOMIC_SUPPORTS_SWAP | MCA_BTL_ATOMIC_SUPPORTS_MIN |
